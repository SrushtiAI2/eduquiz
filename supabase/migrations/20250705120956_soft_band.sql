/*
  # Complete database schema setup for AI Test Generator

  1. New Tables
    - `profiles` - User profile information linked to auth.users
    - `test_papers` - AI-generated test papers with metadata
    - `questions` - Individual questions belonging to test papers

  2. Security
    - Enable RLS on all tables
    - Add policies for user data isolation
    - Secure storage policies for document uploads
  ...
  4. Storage
    - Documents bucket for PDF uploads
*/
...
-- Create test papers table if it doesn't exist
...
BEGIN
  ...
  -- Test papers policies
  DROP POLICY IF EXISTS "Users can view their own test papers" ON public.test_papers;
  DROP POLICY IF EXISTS "Users can create their own test papers" ON public.test_papers;
  DROP POLICY IF EXISTS "Users can update their own test papers" ON public.test_papers;
  DROP POLICY IF EXISTS "Users can delete their own test papers" ON public.test_papers;
  
  -- Questions policies
  DROP POLICY IF EXISTS "Users can view questions for their test papers" ON public.questions;
  DROP POLICY IF EXISTS "Users can create questions for their test papers" ON public.questions;
  DROP POLICY IF EXISTS "Users can update questions for their test papers" ON public.questions;
  DROP POLICY IF EXISTS "Users can delete questions for their test papers" ON public.questions;
END $$;
